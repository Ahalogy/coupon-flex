...
// @input Asset.Texture skrrTexture
// @ui {"widget":"group_end"}
...
// @input Asset.AudioTrackAsset skrrSound
// @ui {"widget":"group_end"}
...
var skrrStore = global.persistentStorageSystem.store;
var skrrKey = "skrr";
...
var idleState = stateMachine.addState({
    name: "Idle",
    onEnter: function() {
        skrrStore.putBool(skrrKey, false);
...
function gameUpdate(eventData) {
    if (!skrrStore.getBool("skrr")) {
      setTexture(script.skrrTexture);
      playSound(script.skrrSound);
      skrrStore.putBool("skrr", true);
    }
